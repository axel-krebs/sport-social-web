...
object Repository extends ProfileComponent {
  ...
  class UserPreferencesSchema(tag: Tag) extends Table[(UserPreferences)](tag, "app_user_prefs") {
    def userKey = column[Long]("user_id") // no primary key here!
  ...
  /*
   * Search a User entry ba name and password; the return value of Option indicates that a possible NULL value may be returned!
   */
  def findUserByNickNameOrEmail(nickNameOrEmail: String): Option[User] = {
    val q = db.run(users.filter(_.nickname === nickNameOrEmail).result.headOption)
